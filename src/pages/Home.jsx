...
export const Home = () => {
  ...
  return (
    <div>
    <div className='bg-whihte mt-2 px-4 md:px-16 lg:px-24 '>
      <div className='container mx-auto py-4 flex flex-col md:flex-row space-x-2'>
          <div className='w-full md:w-3/12'>
            <div className='bg-red-600 text-white text-xs font-bold px-2 py-2.5'>SHOP BY CATEGORIES</div>
            <ul className='space-y-4 bg-gray-100 o-3 border'>
            {Categories.map((category,index)=>(
              <li key={index} className='flex item-center text-sm font-medium'><div className='w-2 h-2 border-red-500 rounded-full mr-2'></div> {category} </li>))}
          </ul>
          </div>
          <div className='w-full md:w-9/12 mt-8 md:mt-0 h-96 relative'>
            <img src={HeroImage} alt="hero image" className='h-full w-full' />
            <div className='absolute top-16 left-8'>
              <p className='text-gray-600 mb-4'>Code with Papa</p>
